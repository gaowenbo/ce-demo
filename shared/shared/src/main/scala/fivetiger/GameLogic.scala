...
import fivetiger._
import fivetiger.model.{ChessState, Step}
...
object GameLogic{
  ...
  def calculateGoal(chessState: ChessState, step: Step) = {
    if (step.action == StepConst.PICK) {
      ...
      var goalMap = scoreMap.getOrElse((step.x, step.y), Array[Int]()).filter(!currentHistoryGoalMap.contains(_)).filter(!scoreList(_)._1.exists(e => !sameCurrentFlag(chessState, e._1, e._2)))
